...
class WBL_Social_Links_Widget extends \Elementor\Widget_Base
{
     ...
     * Cached to prevent repeated database queries
     */
    ...
    protected function render()
    {
        ...
        if (!$social_set_id) {
            if (\Elementor\Plugin::$instance->editor->is_edit_mode()) {
                echo '<div class="wbl-elementor-notice" style="padding:20px;background:#f3f4f6;border-left:4px solid #3b82f6;border-radius:4px"><p style="margin:0;color:#374151"><strong>' . esc_html__('Please select a social set', 'website-bio-link') . '</strong></p><p style="margin:8px 0 0;color:#6b7280;font-size:14px">' . esc_html__('Go to Content → Social Set and choose which set to display.', 'website-bio-link') . '</p></div>';
            }
            return;
        }

        // Get social items with error checking - use single meta query
        $social_items = get_post_meta($social_set_id, '_sky_social_items', true);

        if (empty($social_items) || !is_array($social_items)) {
            if (\Elementor\Plugin::$instance->editor->is_edit_mode()) {
                echo '<div class="wbl-elementor-notice" style="padding:20px;background:#fef3c7;border-left:4px solid #f59e0b;border-radius:4px"><p style="margin:0;color:#92400e"><strong>' . esc_html__('This social set is empty', 'website-bio-link') . '</strong></p><p style="margin:8px 0 0;color:#78350f;font-size:14px">' . esc_html__('Please add social links to this set in WordPress admin.', 'website-bio-link') . '</p></div>';
            }
            return;
        }
    ...
    private function render_fallback_social_list($social_items, $args)
    {
        ...
        foreach ($social_items as $item) {
            ...
            if ($platform_data && isset($platform_data['svg_icon'])) {
                // Use SVG icon only
     ...
     * Optimized with static rendering to prevent duplicate styles
     */
    private function render_hover_animation_styles($animation)
    {
        // Track rendered animations to prevent duplicates
        static $rendered_animations = array();

        if ($animation === 'none' || in_array($animation, $rendered_animations)) {
            return;
        }

        // Mark as rendered
        $rendered_animations[] = $animation;

        // Use minimal CSS - animations are already in style.css
        // Only output if absolutely necessary
        $css_map = array(
            'grow' => '.wbl-hover-grow .wbl-social-link:hover .wbl-social-icon-wrapper{transform:scale(1.1)}',
            'shrink' => '.wbl-hover-shrink .wbl-social-link:hover .wbl-social-icon-wrapper{transform:scale(0.9)}',
            'pulse' => '@keyframes wbl-pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}.wbl-hover-pulse .wbl-social-link:hover .wbl-social-icon-wrapper{animation:wbl-pulse .5s ease-in-out}',
            'bounce' => '@keyframes wbl-bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}}.wbl-hover-bounce .wbl-social-link:hover .wbl-social-icon-wrapper{animation:wbl-bounce .5s ease}',
            'float' => '.wbl-hover-float .wbl-social-link:hover .wbl-social-icon-wrapper{transform:translateY(-5px)}',
            'rotate' => '.wbl-hover-rotate .wbl-social-link:hover .wbl-social-icon-wrapper{transform:rotate(360deg)}',
            'wobble' => '@keyframes wbl-wobble{0%,100%{transform:rotate(0)}25%{transform:rotate(-10deg)}50%{transform:rotate(10deg)}75%{transform:rotate(-5deg)}}.wbl-hover-wobble .wbl-social-link:hover .wbl-social-icon-wrapper{animation:wbl-wobble .5s ease}',
        );

        if (isset($css_map[$animation])) {
            echo '<style>' . $css_map[$animation] . '</style>';
        }
    }
